...
- Import/Export Teamwork data:
  - If you have team members doing their own coding and markup *of the same data files*, you can use the Export/Import Teamwork tool to share "document level data"[^1]
- Merging projects:
  - This option merges together all data from both project files that are merged into a single project. This option works well for cases where you have team members analyzing separate documents that you want to bring into the same project file.[^2]

[^1]: [MAXQDA Manual: Copy Codings, Memos, Variables etc Into Another Project](https://www.maxqda.com/help/teamwork/transferring-coded-segments-memos-variables-etc-from-one-project-to-another)

[^2]: [MAXQDA Manual: Merge Two MAXQDA Projects](https://www.maxqda.com/help/teamwork/merging-two-maxqda-projects)

## Key considerations for Teamwork in MAXQDA

- All team members need to be using the same version of MAXQDA for files to import/export or merge correctly.
- If you intend to measure inter-coder reliability, you must follow the process as described by MAXQDA for this to work correctly. I *highly recommend* trying a test version of your process with a sample of your data to ensure that you have the correct procedure in place.[^3]
- If you have a team member without current access to MAXQDA, they can still see the data and analysis work by using the [free MAXQDA reader program](https://www.maxqda.com/products/maxqda-reader). Using the reader allows a researcher to see how the data has been coded without making any changes to the data or coding.

[^3]: [MAXQDA Manual: Intercoder Agreement](https://www.maxqda.com/help/coding/problem-intercoder-agreement-qualitative-research)

## Exporting to other QDAS programs

- The REFI-QDA standard allows some interoperability between qualitative data analysis software programs (such as MAXQDA and Atlas.ti).[^4]

- You could use this option if:

  - You have completed analysis in another program that you no longer have access to.

  - You have team members at other institutions without access to MAXQDA.

  - You want to share your data for analysis by other researchers or teams (secondary analysis).

[^4]: [MAXQDA Manual: Export and Import REFI-QDA Projects](https://www.maxqda.com/help/report-and-export/export-and-import-refi-qda-projects)
...
::::: columns
::: {.column width="40%"}
MAXQDA keeps larger data files in a separate folder that allows the data to be accessible for analysis, but without making the project file itself too large. The external file location is the same for all MAXQDA projects, but can be a location you choose.[^5]
...
[^5]: [MAXQDA Manual: External Files](https://www.maxqda.com/help/import/external-files)
...
If you have existing transcript files without time stamps or a linked video or audio file, you can upload these to MAXQDA in any of the acceptable text formats.[^6]

[^6]: [MAXQDA Manual: Supported data types](https://www.maxqda.com/help/import/data-types)
...
You can also upload transcripts with time stamps and link them with the recording of conversation.[^7] These transcripts can include the files you get from Zoom or Teams (in the .vtt file format).

[^7]: [MAXQDA Manual: Importing Transcripts](https://www.maxqda.com/help/import/transcripts)
...
If you do not have transcripts, or draft transcripts, you can use the transcription feature in MAXQDA to create or edit a transcript file.[^8]

[^8]: [MAXQDA Manual: Transcription Mode](https://www.maxqda.com/help/transcription-audio-video/transcription-mode)
...
3.  Choose `Import only` from the three options provided. If you choose `Link Transcript` you'll be able to upload an existing transcript, while `Transcribe` opens MAXQDA's auto transcription tool.[^9] This is a paid feature not included in the Illinois license.
:::
...
[^9]: [MAXQDA Manual: Automatic Transcription](https://www.maxqda.com/help/transcription-audio-video/automatic-transcription/transcribe-in-maxqda)
...
MAXQDA can be used to analyze survey data with open-ended responses, or any data structured in a spreadsheet format.[^10]

[^10]: [MAXQDA Manual: Survey Data from Excel](https://www.maxqda.com/help/import/survey-data-and-structured-data-from-excel)
